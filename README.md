...
解决了直接打印 Gemini 页面时出现的滚动截断、元素遮挡、懒加载内容缺失等问题。可以在目录下查看生成的 PDF 和 mhtml 的效果。

该方法既不需要 html 的大部分资源文件（都收紧到一个简单的 mhtml 文件中），也满足了高质量 PDF 的需求。

从原理上看，任何带滚动条的页面都可以试试，不仅仅局限于 Gemini。但是不保证成功～ 如果有有钱人资助我一个 GPT 的会员的话我也可以试试给 GPT 做做适配😋。

## 常见问题
在点击软件的【开始运行】后提示“链接不存在”，可以在左下角看到，这是由于代理不稳定导致的。关闭页面后，重新点击【开始运行】，直至期望的对话内容出现即可。
<img width="1274" height="719" alt="db1ae9a72a44b113726e2f5d861155da" src="https://github.com/user-attachments/assets/350a787d-2219-45d4-b902-e11f7e2dbc83" />
...
### v1.0.1 (2025-01-08)
*   **修复**：修复组件检查功能，现在开始时会自动检查组件是否完备，而不是总是显示【需要下载组件】的状态。
*   **优化**：滚动次数可自定义，一般而言20次都是够了，更多的次数只是在浪费时间。如示例中的那种超级长对话才需要30次。

### v1.0.2 (2025-01-21)
*   **修复**：修复 MacOS 下，组件检测工作不正常的问题。现在 MacOS 和 Windows 都将下载目录固定在本地，而不是临时目录。
